...
struct Pertag {
	...
	int gappxs[LENGTH(tags) + 1]; /* gaps per tag */
...
{
	selmon->gappx = selmon->pertag->gappxs[selmon->pertag->curtag] = 7;
	arrange(selmon);
}
...
setgaps(const Arg *arg)
{
	if ((arg->i == 0) || (selmon->gappx + arg->i < 0))
		selmon->gappx = selmon->pertag->gappxs[selmon->pertag->curtag] = 0;
	else
		selmon->gappx = selmon->pertag->gappxs[selmon->pertag->curtag] += arg->i;
	arrange(selmon);
}
